...
Написал небольшой скрипт на питоне чтобы было удобней собирать и воспроизвести окружение для запуска benchmark'ов. Запуск: *./cmd [dep]*:
...
check | пройти тесты (спойлер: все ок)
...
Некоторые benchmark'и все еще не собираются на корректных патчах (ошибка в пакетах), но результаты по локализации и генерации патчей такие же, как в статье.
![prophet-res](https://github.com/StasBel/prophet-test/blob/master/doc/prophet-res.png)

Средний номер корректого патча из простанства сгенерируемых - 11 из 100.

Срдний номер корректного из "plausible" - 1 с чем-то.
